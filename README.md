...
4. **ビルドのみ実行（エクスポートなし）**
   静的エクスポートを行わず、ビルド（コンパイルと型チェック）の成功確認のみを行いたい場合は、以下のコマンドを使用します：

   ```bash
   pnpm build:no-export
   ```
...
※ エクスポートを行わず、ビルドの確認のみを行いたい場合は `pnpm run build:no-export` を使用してください。
...
4. バージョン情報ファイル（`version.json`）を生成
...
デプロイされたコンテンツのバージョン（ビルド日時）は以下の方法で確認できます：

1. **ブラウザのコンソール**:
   開発者ツールを開くと、コンソールに以下のような形式でビルド日時が表示されます。
   `Build Info 2024/12/24 10:00:00 dev (a1b2c3d)`
   （日時、ブランチ名、コミットハッシュ）

2. **バージョン情報ファイル**:
   以下の URL にアクセスすることで、JSON 形式でビルド情報を確認できます。
   `https://industrial-art.sd.tmu.ac.jp/ge2026/version.json`

   ```json
   {
     "buildTime": "2024/12/24 10:00:00",
     "git": {
       "branch": "dev",
       "sha": "a1b2c3d"
     },
